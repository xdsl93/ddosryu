...
class NetworKMainDDos(app_manager.RyuApp):
    """
        MainDDos is a Ryu app for forwarding packets in shortest
        path.
    ...
    _CONTEXTS = {
        "network_topo_disc": network_topo_disc.NetworkTopoDisc,
    ...
    def __init__(self, *args, **kwargs):
        super(NetworKMainDDos, self).__init__(*args, **kwargs)
        self.name = 'network_main_ddos'
        self.topo_disc = kwargs["network_topo_disc"]
    ...
    def set_weight_mode(self, weight):
        ...
        if self.weight == self.WEIGHT_MODEL['hop']:
            self.topo_disc.get_shortest_paths(weight=self.weight)
        return True
    ...
    def flood(self, msg):
        ...
        for dpid in self.topo_disc.access_ports:
            for port in self.topo_disc.access_ports[dpid]:
                if (dpid, port) not in self.topo_disc.access_table.keys():
    ...
    def arp_forwarding(self, msg, src_ip, dst_ip):
        ...
        result = self.topo_disc.get_host_location(dst_ip)
    ...
    def get_path(self, src, dst, weight):
        """
            Get shortest path from network topo_disc module.
        """
        shortest_paths = self.topo_disc.shortest_paths
        ...
        if weight == self.WEIGHT_MODEL['hop']:
            ...
            try:
                ...
                paths = self.topo_disc.k_shortest_paths(graph, src, dst,
                                                        weight=weight)
    ...
    def get_sw(self, dpid, in_port, src, dst):
        ...
        src_location = self.topo_disc.get_host_location(src)
        if in_port in self.topo_disc.access_ports[dpid]:
            if (dpid,  in_port) == src_location:
                src_sw = src_location[0]
            else:
                return None

        dst_location = self.topo_disc.get_host_location(dst)
        if dst_location:
    ...
    def network_main_ddos(self, msg, eth_type, ip_src, ip_dst):
        """
        ...
        if result:
            ...
            if dst_sw:
                ...
                self.install_flow(self.datapaths,
                                  self.topo_disc.link_to_port,
                                  self.topo_disc.access_table, path,
    ...
    def _packet_in_handler(self, ev):
        ...
        if isinstance(ip_pkt, ipv4.ipv4):
            ...
            if len(pkt.get_protocols(ethernet.ethernet)):
                ...
                self.network_main_ddos(msg, eth_type, ip_pkt.src, ip_pkt.dst)