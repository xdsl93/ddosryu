...
class NetworkDelayDetector(app_manager.RyuApp):
    ...
    def get_delay(self, src, dst):
        ...
        """
        try:
            fwd_delay = self.topo_disc.graph[src][dst]['lldpdelay']
            re_delay = self.topo_disc.graph[dst][src]['lldpdelay']
    ...
    def create_link_delay(self):
        ...
        """
        try:
            for src in self.topo_disc.graph:
                for dst in self.topo_disc.graph[src]:
                    if src == dst:
                        self.topo_disc.graph[src][dst]['delay'] = 0
                        continue
                    delay = self.get_delay(src, dst)
                    self.topo_disc.graph[src][dst]['delay'] = delay
        except:
            if self.topo_disc is None:
                self.topo_disc = lookup_service_brick('topo_disc')
            return
    ...
    def show_delay_statis(self):
        if setting.TOSHOW and self.topo_disc is not None:
            self.logger.info("\nsrc   dst      delay")
            self.logger.info("---------------------------")
            for src in self.topo_disc.graph:
                for dst in self.topo_disc.graph[src]:
                    delay = self.topo_disc.graph[src][dst]['delay']
